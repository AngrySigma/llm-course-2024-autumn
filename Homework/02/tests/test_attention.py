...
class TestAttention(unittest.TestCase):
    ...
    def test_multihead(self):
        ...
        q_proj = (
            (q.transpose(0, 1) @ w_q.T)
            .view(SEQ_LENGTH, BATCH_SIZE * N_HEADS, DIM_PER_HEAD)
            .transpose(0, 1)
        )
        k_proj = (
            (k.transpose(0, 1) @ w_k.T)
            .view(SEQ_LENGTH, BATCH_SIZE * N_HEADS, DIM_PER_HEAD)
            .transpose(0, 1)
        )
        v_proj = (
            (v.transpose(0, 1) @ w_v.T)
            .view(SEQ_LENGTH, BATCH_SIZE * N_HEADS, DIM_PER_HEAD)
            .transpose(0, 1)
        )
        ...
        self.assertTrue(torch.allclose(default_mha, custom_mha, atol=1e-7))
